...
from Inleveropdracht_2.doolhof import Maze
from Inleveropdracht_2.policy import Policy
...
class Agent:
    ...
    def episode(self, startstate, policy):
        ...
        # state = random.choice([state for state in self.maze.states if not state.is_endstate])
    ...
    def monte_carlo_policy_evaluation_main_loop(self, beginstate, gamma):
        ...
        while k != 500:
            ...
            for state in range(1, len(episode)):
                ...
                if episode[state] not in episode[:state]:
    ...
    def monte_carlo_policy_evaluation(self, beginstate, gamma):
        returns, values = self.monte_carlo_policy_evaluation_main_loop(beginstate, gamma)
